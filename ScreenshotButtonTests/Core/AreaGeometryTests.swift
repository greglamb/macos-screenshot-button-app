...
struct AreaGeometryTests {
    @Test(
        "Rectangle normalizes regardless of drag direction",
        arguments: [
            DragPair(
                start: .init(x: 0, y: 0), end: .init(x: 10, y: 10), expected: .init(x: 0, y: 0, width: 10, height: 10)),
            DragPair(
                start: .init(x: 10, y: 10), end: .init(x: 0, y: 0), expected: .init(x: 0, y: 0, width: 10, height: 10)),
            DragPair(
                start: .init(x: 5, y: 20), end: .init(x: 25, y: 0), expected: .init(x: 5, y: 0, width: 20, height: 20)),
            DragPair(
                start: .init(x: 3, y: 3), end: .init(x: 3, y: 3), expected: .init(x: 3, y: 3, width: 0, height: 0)),
        ])
    ...
    @Test(
        "Drags below the 5pt threshold are treated as cancel",
        arguments: [
