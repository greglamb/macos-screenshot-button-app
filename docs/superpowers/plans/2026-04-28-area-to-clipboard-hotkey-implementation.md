...
> **POSTSCRIPT — corrections from manual verification (2026-04-29):** Tasks 1–11 landed as written, but Task 13's manual verification surfaced three bugs that this plan's specified code did not anticipate. The shipped code corrects: (1) TCC bucket — `NSEvent.addGlobalMonitorForEvents` for keyboard events requires **Accessibility** (`AXIsProcessTrustedWithOptions`), not Input Monitoring (`IOHIDCheckAccess`/`IOHIDRequestAccess`), per Apple's docs; (2) `SettingsLink` does not activate `LSUIElement = true` apps — paired with `simultaneousGesture` activate; (3) strict `mods.isEmpty` modifier check rejected `Fn`, breaking Fn+F-key on default Mac keyboards — now only rejects `[.command, .option, .control, .shift]`. See commits `3ad9f73` and `50760fe` for the actual implementation. The plan body below is preserved as the historical record of what was originally executed.
